...
function getNombrePieza(fdi: number) {
  if (fdi <= 0) return '';
  const unidad = fdi % 10;
  const cuadrante = Math.floor(fdi / 10);
  const ubicacion = (cuadrante === 1 || cuadrante === 2 || cuadrante === 5 || cuadrante === 6) ? 'Sup.' : 'Inf.';

  let tipo = '';
  if (unidad === 1 || unidad === 2) tipo = 'Incisivo';
  else if (unidad === 3) tipo = 'Canino';
  else if (unidad === 4 || unidad === 5) tipo = 'Premolar';
  else if (unidad >= 6 && unidad <= 8) tipo = 'Molar';

  return `${tipo} ${ubicacion}`;
}

function IconoDiente({ tipo, className }: { tipo: string, className?: string }) {
  const tipoBase = tipo.split(' ')[0];

  const paths = {
    Incisivo: "M8 3h8v5c0 6-2 13-4 13s-4-7-4-13V3z",
    Canino:   "M12 2l4 6c0 6-2 13-4 13s-4-7-4-13l4-6z",
    Premolar: "M7 4c0-2 2-2 5-1 3-1 5-1 5 1v5c0 3-1 6-2 8v4c0 1.5-1.5 3-3 3s-3-1.5-3-3v-4c-1-2-2-5-2-8V4z",
    Molar:    "M6 4c0-2 3-2 5-1 2-1 5-1 5 1v6c0 2-1 4-1 6v3c0 1.5-1.5 3-3 3s-3-1.5-3-3v-3c0-1-2-1-2 0v3c0 1.5-1.5 3-3 3s-3-1.5-3-3v-3c0-2-1-4-1-6V4z"
  };

  const d = paths[tipoBase as keyof typeof paths] || paths.Molar;
...
function obtenerActualizacionDiente(nombreTratamiento: string): Partial<DienteEstado> {
  const n = nombreTratamiento.toLowerCase();

  const actualizacion: Partial<DienteEstado> = {
    estadoClinico: 'REALIZADO',
    colorRecuadro: 'AZUL'
  };

  // ── 1. Coronas Modernas y Clásicas ──
  if (n.includes('corona')) {
    actualizacion.trazoCorona = 'CIRCULO_AZUL';

    if (n.includes('zirconio')) actualizacion.siglaRecuadro = 'CZ';
    else if (n.includes('disilicato')) actualizacion.siglaRecuadro = 'CDL';
    else if (n.includes('ceromero') || n.includes('cerómero')) actualizacion.siglaRecuadro = 'CC';
    else if (n.includes('metal') && n.includes('porcelana')) actualizacion.siglaRecuadro = 'CMP';
    else if (n.includes('metal')) actualizacion.siglaRecuadro = 'CM';
    else if (n.includes('porcelana')) actualizacion.siglaRecuadro = 'CP';
    else if (n.includes('acrilico') || n.includes('acrílico') || n.includes('provisional')) actualizacion.siglaRecuadro = 'C_PROV';
    else actualizacion.siglaRecuadro = 'C';
  }

  // ── 2. Incrustaciones ──
  else if (n.includes('incrustaci')) {
    if (n.includes('ceromero') || n.includes('cerómero')) actualizacion.siglaRecuadro = 'INC';
    else if (n.includes('disilicato') || n.includes('estetica') || n.includes('estética')) actualizacion.siglaRecuadro = 'IE';
    else if (n.includes('metal')) actualizacion.siglaRecuadro = 'IM';
    else actualizacion.siglaRecuadro = 'IN';
  }

  // ── 3. Resinas y Restauraciones ──
  else if (n.includes('resina')) {
    if (n.includes('nano')) actualizacion.siglaRecuadro = 'RNH';
    else actualizacion.siglaRecuadro = 'R';
  }
  else if (n.includes('amalgama')) actualizacion.siglaRecuadro = 'AM';
  else if (n.includes('ionomero') || n.includes('ionómero')) actualizacion.siglaRecuadro = 'IV';

  // ── 4. Tratamientos Radiculares / Quirúrgicos ──
  if (n.includes('endodoncia')) {
    actualizacion.siglaRecuadro = 'TC';
    actualizacion.trazoRaiz = 'LINEA_VERTICAL_AZUL';
  }
  else if (n.includes('extraccion') || n.includes('extracción') || n.includes('exodoncia')) {
    actualizacion.condicionGeneral = 'EXTRAIDO';
    actualizacion.trazoCorona = 'X_AZUL';
    actualizacion.colorRecuadro = '';
    actualizacion.siglaRecuadro = '';
  }
  else if (n.includes('implante')) {
    actualizacion.siglaRecuadro = 'IMP';
  }
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  {item.numeroFdi > 0 && (
                    <span className="inline-flex items-center gap-1.5 text-xs font-bold text-blue-700 bg-blue-50 px-2.5 py-1 rounded-md border border-blue-200 shadow-sm">
                      <IconoDiente tipo={getNombrePieza(item.numeroFdi)} className="w-3.5 h-3.5 text-blue-500 shrink-0" />
                      Pieza {item.numeroFdi}
                      <span className="text-blue-500/80 font-semibold px-1">|</span>
                      <span className="text-blue-500 font-medium">{getNombrePieza(item.numeroFdi)}</span>
                    </span>
                  )}
        ...
        {evoluciones.length === 0 ? (
          ...
          <div className="relative px-4 py-4">
            ...
            <div className="space-y-4">
              {evoluciones.map((ev) => {
                const match = ev.descripcion?.match(/^\[(.*?)\]\s*(.*)$/s);
                const tituloTratamiento = match ? match[1] : (ev.numeroFdi ? 'Tratamiento Registrado' : 'Evolución Clínica General');
                const notaClinica = match ? match[2] : ev.descripcion;
                ...
                return (
                  <div key={ev.idEvolucion} className="relative flex gap-4 pl-10">
                    ...
                    <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all duration-150 overflow-hidden">
                      ...
                      {/* Header de la Evolución (Título + Pieza) */}
                      <div className="px-4 pt-3.5 pb-2 border-b border-slate-50 flex items-start justify-between gap-4">
                        <h4 className="text-sm font-bold text-slate-800 leading-tight">
                          {tituloTratamiento}
                        </h4>
                        {ev.numeroFdi && ev.numeroFdi > 0 && (
                          <span className="shrink-0 inline-flex items-center gap-1.5 text-[11px] font-bold text-blue-700 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-lg">
                            <IconoDiente tipo={getNombrePieza(ev.numeroFdi)} className="w-3 h-3 shrink-0" />
                            Pieza {ev.numeroFdi} <span className="font-normal opacity-75 hidden sm:inline">({getNombrePieza(ev.numeroFdi)})</span>
                          </span>
                        )}
                      </div>
                      ...
                      <div className="px-4 py-3">
                        <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">
                          {notaClinica || 'Sin nota clínica detallada.'}
                        </p>
                      </div>
                      ...
                      <div className="px-4 py-2.5 bg-slate-50 border-t border-slate-100 flex items-center gap-2">
                        ...
                        <p className="text-xs text-slate-500 truncate">
                          ...
                          <span className="font-medium text-slate-500">{ev.fecha}</span>
                        </p>
                      </div>
                    </div>
                  </div>
...
export default function EvolucionesTab({ idPaciente, idCiclo }: { idPaciente: number; idCiclo: number }) {
  ...
  const handleConfirmar = async () => {
    ...
    setSaving(true);
    try {
      const notaEstructurada = `[${modalItem.nombreTratamiento}] ${nota.trim()}`;
      ...
      if (modalItem.numeroFdi > 0 && idOdontogramaActual) {
        ...
        const actualizacionesVisuales = obtenerActualizacionDiente(modalItem.nombreTratamiento);

        await odontogramaService.guardarDiente({
          ...dienteTarget,
          idOdontograma: idOdontogramaActual,
          ...actualizacionesVisuales
        });
      }

      // Recargar listas y el Odontograma actualizado (respetando el idCiclo)
